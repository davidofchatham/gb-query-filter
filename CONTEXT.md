...
**Automated coverage exists for the targeting rule and custom-field ownership:** the `query-filters` fixture blueprint at [tools/fixtures/query-filters/](tools/fixtures/query-filters/) — see its README. It seeds a real Meta Box field and a real ACF field (not mocks), so both integration branches are exercised end to end. Taxonomy filtering, the AJAX path, and the JS-side targeting rule in `gbqf-frontend.js` are **not** covered — the blueprint keeps `enableAjax` false throughout so a JS-side decision cannot colour a PHP-side result. Everything else is manual.
